...
exports.getSetupMessage = async (message, object, messageNr) => {
	...
	try {
		switch (messageNr) {
		case 1: return `${Formatters.bold(Formatters.italic('Psst.. over here ' + Formatters.userMention(message.guild.ownerId)))}`;

		case 2: return `${Formatters.bold(Formatters.italic('Come closer... ' + Formatters.userMention(message.guild.ownerId)))}`;

		case 3: return `${Formatters.bold('Finally...')} took you a while!\n\nAnyways, Hello, I am ${Formatters.bold(config.client.name)}.\n\nI was told that ${Formatters.bold(message.guild.name)} is in need of a Discord bot and since you seem to be in charge of here, I've contacted you to get things rolling.\n\nFor me to serve ${Formatters.bold(message.guild.name)}, you as the leader need to prove me that you are worthy. To do that, I need you to complete a few quests. Before you start panicking, the quests are not hard..., just carefully listen to my instructions.\n\nAre you ready to start? ${Formatters.italic('(type ' + Formatters.bold('yes') + ')')}`;
